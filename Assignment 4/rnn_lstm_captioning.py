...
class CaptioningRNN(nn.Module):
    ...
    def __init__(
        ...
        # Project CNN features to h0 (for rnn/lstm) or A (for attn, 4x4 spatial)
        if cell_type in ("rnn", "lstm"):
            # global average pool → h0
            self.feature_proj = nn.Linear(input_dim, hidden_dim)
        else:
            # spatial features → A of shape (N, H, 4, 4)
            self.feature_proj = nn.Linear(input_dim, hidden_dim)
    ...
    def forward(self, images, captions):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        # (1) Extract image features and project
        features = self.encoder(images)  # (N, C, 4, 4)
        N = features.shape[0]
        if self.cell_type in ("rnn", "lstm"):
            # global average pool
            features_avg = features.mean(dim=(2, 3))  # (N, C)
            h0 = self.feature_proj(features_avg)       # (N, H)
            # (2) Word embeddings
            word_vecs = self.embedding(captions_in)    # (N, T, W)
            # (3) RNN/LSTM
            hn = self.rnn(word_vecs, h0)               # (N, T, H)
        else:  # attn
            # project spatial features to (N, H, 4, 4)
            # features: (N, C, 4, 4) -> reshape to (N, 16, C) -> linear -> (N, 16, H) -> (N, H, 4, 4)
            C = features.shape[1]
            feats_flat = features.permute(0, 2, 3, 1).reshape(N, -1, C)  # (N, 16, C)
            A = self.feature_proj(feats_flat).reshape(N, 4, 4, -1).permute(0, 3, 1, 2)  # (N, H, 4, 4)
            word_vecs = self.embedding(captions_in)    # (N, T, W)
            hn = self.rnn(word_vecs, A)                # (N, T, H)

        # (4) Output projection
        scores = self.output_proj(hn)  # (N, T, V)
        # (5) Loss
    ...
    def sample(self, images, max_length=15):
        ...
        if self.cell_type in ("rnn", "lstm"):
            features_avg = features.mean(dim=(2, 3))  # (N, C)
            h = self.feature_proj(features_avg)        # (N, H)
        ...
        for t in range(max_length):
            x_t = self.embedding(word)  # (N, W)
            if self.cell_type == "rnn":
                h = self.rnn.step_forward(x_t, h)
            elif self.cell_type == "lstm":
                h, c = self.rnn.step_forward(x_t, h, c)
            else:  # attn
                attn, attn_weights = dot_product_attention(h, A)
                h, c = self.rnn.step_forward(x_t, h, c, attn)

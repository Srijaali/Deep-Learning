...
def prepocess_input_sequence(
    ...
    for token in tokens:
        if token in spc_tokens:
            ...
            # treat as a number, break into individual digit characters
            for ch in token:
...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        ...
        for i in range(K):
            for j in range(K):
                scores[i, j] = torch.dot(query[n, i], key[n, j]) / (M ** 0.5)
        weights = F.softmax(scores, dim=1)  # (K, K)
...
def get_subsequent_mask(seq):
    ...
    # Replace "pass" statement with your code
    N, K = seq.shape
    # Upper triangular (excluding diagonal) = True (masked)
    mask = torch.triu(torch.ones(K, K, dtype=torch.bool, device=seq.device), diagonal=1)
    mask = mask.unsqueeze(0).expand(N, -1, -1)  # (N, K, K)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    y = pos.unsqueeze(1).expand(K, M).unsqueeze(0)   # (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    dim = torch.arange(M, dtype=torch.float32).unsqueeze(0)   # (1, M)
    angles = pos / (10000 ** (2 * (dim // 2) / M))            # (K, M)
    pe = torch.zeros(K, M)
    pe[:, 0::2] = torch.sin(angles[:, 0::2])
    pe[:, 1::2] = torch.cos(angles[:, 1::2])
    y = pe.unsqueeze(0)  # (1, K, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        # mask for the decoder: based on ans_b[:, :-1] which matches a_emb_inp length
